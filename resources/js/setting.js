$(document).ready(function() {
    ...
    function drawSkillsTable(skills) {
        ...
        skills.forEach(function(skill) {
            skill.sub_skills.ids.forEach(function(sub_skill_id, idx) {
                ...
                if (idx == 0) {
                    newRow += '<td class="text-center text-white pl-2 pr-2 align-middle" rowspan="' + 
    ...
    $('#settings-tab-skills #btn-save-main-skill-settings').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#settings-tab-skills select#main_skill').append(newOption);

                // Set focus to main skill name input
                $('#settings-tab-skills input#main_skill_name').val('');
                $('#settings-tab-skills input#main_skill_name').focus();
    ...
    $('#settings-tab-skills #btn-save-sub-skill-settings').on('click', function() {
        if ($('#settings-tab-skills select#main_skill').val() == undefined ||
            $('#settings-tab-skills select#main_skill').val() == '') {
            alert('Please choose main skill!');
            $('#settings-tab-skills select#main_skill').focus();
            return;
        }

        if ($('#settings-tab-skills input#sub_skill_name').val() == undefined ||
    ...
    $(document).on('click', '#settings-tab-skills table#skills-table tbody tr button.btn-remove-main-skill', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Remove row from main skills select
    ...
    $('#edit-main-skill-modal #btn-update-main-skill').on('click', function() {
        if ($('#edit-main-skill-modal input#edit_main_skill_name').val() == undefined ||
            $('#edit-main-skill-modal input#edit_main_skill_name').val() == '') {
            alert('Please enter main skill name!');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Update main skill select
                $('#settings-tab-skills select#main_skill option').each(function() {
                    if ($(this).val() == response.skill.id) {
                        $(this).text(response.skill.name);
                        return;
                    }
                });

                // Hide Edit Main Skill Modal
                $('#edit-main-skill-modal').modal('hide');

                // Show message
                $('.toast .toast-header').removeClass('bg-danger');
                $('.toast .toast-header').addClass('bg-success');
                $('.toast .toast-body').text('Main Skill (' + response.skill.name + ') was updated successfully!');
                $('.toast').toast('show');
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                loader('hide');
    ...
    $(document).on('click', '#settings-tab-skills table#skills-table tbody tr button.btn-edit-sub-skill', function() {
        ...
        // Copy main skills
        $('#edit-sub-skill-modal select#edit_sub_skill_p_id').html($('#settings-tab-skills select#main_skill').html());
        $('#edit-sub-skill-modal select#edit_sub_skill_p_id').val(skillPId);
