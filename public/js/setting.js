...
$(document).ready(function () {
  ...
  function clearEditSubSkillModal() {
    ...
    $('#edit-sub-skill-modal select#edit_sub_skill_p_id').html(''); // $('#edit-sub-skill-modal select#edit_sub_skill_p_id').selectpicker('refresh');
  }
  ...
  $(document).on('click', '#settings-tab-skills table#skills-table tbody tr button.btn-edit-sub-skill', function () {
    ...
    $('#edit-sub-skill-modal select#edit_sub_skill_p_id').html(newOptions); // $('#edit-sub-skill-modal select#edit_sub_skill_p_id').selectpicker('refresh');
  ...
  $('button#btn-save-task-event').on('click', function () {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.success) {
          // Show message
          showMessage('success', response.task_event.name + ' was saved successfully!'); // Remove no-data row

          $('table#task-event-table tbody tr.no-data').remove(); // Add it into task event table
          ...
          innerHtml += '</td>';
          innerHtml += '</tr>';
          $('table#task-event-table tbody').append(innerHtml); // Initalize task event

          $('#settings-tab-opportunities input#task-event').val(''); // Set focus to task event
  ...
  $(document).on('click', 'button.btn-remove-task-event', function () {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.success) {
          // Show message
          showMessage('success', teName + ' was removed successfully!'); // Remove tr
  ...
  $(document).on('click', 'button.btn-edit-task-event', function () {
    ...
    $('#edit-task-event-modal input[name=edit-task-event-name]').val(teName); // Show Edit Task / Event Modal
  ...
  $('button#btn-update-task-event').click(function () {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.success) {
          // Show message
          showMessage('success', response.task_event.name + ' was updated successfully!'); // Hide modal

          $('#edit-task-event-modal').modal('hide'); // Update tr
