...
$(document).ready(function () {
  ...
  $('#btn-create-new-tab').click(function () {
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        initializeDatePicker(); // Add opportunity id and name
  ...
  $('#add-task-modal button#btn-create-new-task').click(function () {
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        $('#oppTabs a#tab-' + idx).find('strong').text('*'); // Draw suggest task modal

        var suggest = res.suggest;

        if (res.suggest.steps.length > 0) {
          drawSuggestTaskModal(suggest); // Show suggest task modal
  ...
  $(document).on('click', '.btn-add-jppsoe-row', function () {
    var jTable = $(this).closest('.tab-component').find('#jppsoe-table'); // Remove no data row
  ...
  $(document).on('change', '.tab-component table#jppsoe-table input, .tab-component table#jppsoe-table select', function () {
    ...
    var no = $(rowObj).find('input[name=no]').val();
    var taskEvent = $(rowObj).find('select[name=task_event]').val();
    var ownership = 0;

    if ($(rowObj).find('input[name=user_company]').is(':checked') == true && $(rowObj).find('input[name=opp_organisation]').is(':checked') == true) {
      ownership = 3;
    } else if ($(rowObj).find('input[name=user_company]').is(':checked') == true && $(rowObj).find('input[name=opp_organisation]').is(':checked') == false) {
      ownership = 1;
    } else if ($(rowObj).find('input[name=user_company]').is(':checked') == false && $(rowObj).find('input[name=opp_organisation]').is(':checked') == true) {
      ownership = 2;
    }

    var targetDate = $(rowObj).find('input[name=target_date]').val();
    var completed = $(rowObj).find('select[name=completed]').val();
    var comments = $(rowObj).find('input[name=comments]').val();
    loader('show');
    $.ajax({
      ...
      success: function success(res) {
        ...
        if (res.success) {
          showMessage('success', 'Data was saved successfully.'); // Change jpp soe id
  ...
  $(document).on('click', '.btn-remove-jppsoe-row', function () {
    ...
    if (id == 0) {
      var jppSoeTable = $(rowObj).closest('table'); // Remove row element
